...
// menuItems - get
// required data: menu item id
// optional data: none
// only allow signed in (authenticated) user to access menu items
handlers._menuItems.get = (data, callback) => {
    // check that the id provided is valid
    var id = typeof(data.queryStringObject.id) == 'string' && data.queryStringObject.id.trim().length == 3 ? data.queryStringObject.id.trim() : false;
    if (id) {
        // lookup menu item
